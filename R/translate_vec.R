...
#'
#' @param text character vector to be translated. Only UTF8-encoded plain text is supported. Elements may contain multiple sentences.
...
translate_vec <- function(text, source_lang = NULL, target_lang = "EN", tag_handling = NULL,
  ...
  if (is.null(source_lang)) {
    ...
    # Define function
    translate_fun_1 <- function(t, tl) translate(text = t, source_lang = source_lang, target_lang = tl,
                                                 tag_handling = tag_handling, split_sentences = split_sentences,
                                                 preserve_formatting = preserve_formatting, get_detect = get_detect,
                                                 auth_key = auth_key)

    if (!get_detect) translation_vec <- purrr::map2_chr(text, target_lang, translate_fun_1)
    if (get_detect) translation_vec <- purrr::map2_df(text, target_lang, translate_fun_1)
  ...
  if (!is.null(source_lang)) {
    ...
    # Define function
    translate_fun_2 <- function(t, sl, tl) translate(text = t, source_lang = sl, target_lang = tl,
                                                     tag_handling = tag_handling, split_sentences = split_sentences,
                                                     preserve_formatting = preserve_formatting, get_detect = get_detect,
                                                     auth_key = auth_key)

    if (!get_detect) translation_vec <- purrr::pmap_chr(list(t = text, sl = source_lang, tl = target_lang), translate_fun_2)
    if (get_detect) translation_vec <- purrr::pmap_df(list(t = text, sl = source_lang, tl = target_lang), translate_fun_2)
